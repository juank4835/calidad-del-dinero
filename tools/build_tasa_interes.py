...
Versión v2: estructura más quirúrgica. Quita el lema y todo el bloque
sobre el ciclo auge/depresión (Rothbard, segunda cita de Mises) — eso
queda para el cap 8 «La asignación intertemporal». Reescribe el cierre
en torno al motivo «el mensaje y el mundo se encontrarán»: la tasa
falsificada NO puede tocar el hecho (la paciencia real, los recursos
realmente libres); solo tuerce la transmisión. Y como solo tuerce la
transmisión, el mensaje y el mundo van a encontrarse —es inevitable—.
...
- 4 citas en bloque: Hayek (3), Mises (1).
- Sin lema (la versión v1 tenía uno).
...
TITLE_TECNICO = "La tasa de interés"
SUBTITLE = "Por qué la tasa de interés no es el precio del dinero"
...
# Sin EXTRA_CSS: esta versión no tiene blockquote.lema.
...
def main():
    ...
    out = re.sub(r'<title>.*?</title>',
                 '<title>El precio del tiempo — Arregla el dinero, arregla el mundo</title>',
    ...
    new_nav = ('<nav class="nav-foot">'
               ...
               '<a class="next" href="asignacion-intertemporal.html">La longitud de la cadena</a></nav>')
